...
"""
    ...
    def test_validate_file_invalid_dag_param(self):
        """Test validate_file with invalid DAG parameter."""
        content = """
dag = DAG(dag_id="test", invalid_param="value")
"""
        file_path = self._create_file(content)

        mock_validator = Mock()
        mock_validator.get_allowed_operators.return_value = []
        mock_validator.validate_field.side_effect = (
            lambda section, field, value, no_value_check: field != "invalid_param"
        )  # noqa: E501

        try:
            with self.assertRaises(InvalidDagParameter) as context:
                validate_file(file_path, mock_validator)
            self.assertIn("Dag parameter 'invalid_param' is not supported", str(context.exception))
        finally:
            os.unlink(file_path)
