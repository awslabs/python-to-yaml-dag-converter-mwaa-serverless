...
def validate_file(file_path: Path, validator: ArgumentValidator):
    ...
    dag_params_in_file = extract_dag_params(file_path)
    for dag_param in dag_params_in_file:
        if not validator.validate_field("dag", dag_param, None, no_value_check=True):
            raise InvalidDagParameter(f"Dag parameter '{dag_param}' is not supported")
