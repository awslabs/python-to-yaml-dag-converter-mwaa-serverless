...
def run_or_skip():
    ...
    return "end_workflow" if botocore_version < (1, 34, 90) else "external_sources_rag"
...
def create_collection(collection_name: str):
    ...
    return aoss_client.conn.create_collection(name=collection_name, type="VECTORSEARCH")["createCollectionDetail"]["id"]
...
def copy_data_to_s3(bucket: str):
    ...
    for source in sources:
        with tempfile.NamedTemporaryFile(mode="w", prefix="") as data_file:
            ...
            S3Hook().conn.upload_file(Filename=data_file.name, Bucket=bucket, Key=os.path.basename(data_file.name))
...
def delete_data_source(knowledge_base_id: str, data_source_id: str):
    ...
    bedrock_agent_client.conn.delete_data_source(dataSourceId=data_source_id, knowledgeBaseId=knowledge_base_id)
...
def delete_opensearch_policies(collection_name: str):
    ...
    access_policies = aoss_client.conn.list_access_policies(type="data", resource=[f"collection/{collection_name}"])[
...
with DAG(
    ...
    chain(
        ...
        EmptyOperator(task_id="end_workflow", trigger_rule=TriggerRule.NONE_FAILED_MIN_ONE_SUCCESS),
        retrieve,
        delete_data_source(
            knowledge_base_id=create_knowledge_base.output,
            data_source_id=create_data_source.output,
